...
int main(int argc, char** argv) {
	...
	while(ros::ok()) {
		...
		ROS_INFO_STREAM(" CURRENT TRANSFORM "<<tf<<endl);
		ft_sensor_value = control.getFTSensorValue();
		ROS_INFO_STREAM("Recieved FT value at tool"<<endl<<ft_sensor_value<<endl);
		ft_sensor_value_transformed = control.getTransformedWrench();
		ROS_INFO_STREAM("Transformed ft value at base "<<endl<<ft_sensor_value_transformed<<endl);
