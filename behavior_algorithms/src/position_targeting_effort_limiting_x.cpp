    ...
    while( (loops_so_far <= total_number_of_loops) && !effort_limit_crossed && !target_reached) {
        ...
        if(TARGET_DISTANCE > 0){
            ...
            if(cutting){
                ...
                virtual_attractor.pose.position.y = current_pose.position.y + task_vector_x.y * PULL_DISTANCE + task_vector_z.x * KEEP_CUTTING_DISTANCE;
                virtual_attractor.pose.position.z = current_pose.position.z + task_vector_x.z * PULL_DISTANCE + task_vector_z.x * KEEP_CUTTING_DISTANCE;
            }
            else{
