...
int main(int argc, char** argv) {
    ...
    */
    while( (loops_so_far <= total_number_of_loops) && !effort_limit_crossed && (((abs(current_pose.position.y) < target_position) && (TARGET_DISTANCE >= 0)) || ((abs(current_pose.position.y) >= target_position) && (TARGET_DISTANCE < 0))) ) {
        // ROS: for communication between programs
        ros::spinOnce();
