...
int main(int argc, char** argv) {
    ...
    ros::Subscriber tool_vector_sub_x = nh.subscribe("tool_vector_x",1,tool_vector_callback);                  // subscribe to the value of the tool vector in the z, published from the accomodation controller
   ...
   // ROS: Wait until we have position data. Our default position is 0.
    while(current_pose.position.x == 0 || tool_vector_x.x == 0) ros::spinOnce();
    ...
    ending_position.x = beginning_position.x + tool_vector_x.x * TARGET_DISTANCE;
    ending_position.y = beginning_position.y + tool_vector_x.y * TARGET_DISTANCE;
    ending_position.z = beginning_position.z + tool_vector_x.z * TARGET_DISTANCE;

    geometry_msgs::Vector3 original_tool_vector_x = tool_vector_x;
    ...
    double dot_product = vector_to_goal.x * original_tool_vector_x.x + vector_to_goal.y * original_tool_vector_x.y + vector_to_goal.z * original_tool_vector_x.z;
    ...
    // Debug output
    cout<<"Tool Vector Z"<<endl<<tool_vector_x<<endl;
    ...
    */
    while( (loops_so_far <= total_number_of_loops) && !effort_limit_crossed && !target_reached) {
        ...
        if(TARGET_DISTANCE > 0){
            virtual_attractor.pose.position.x = current_pose.position.x + tool_vector_x.x * PULL_DISTANCE + tool_vector_z.x * KEEP_CUTTING_DISTANCE; 
            virtual_attractor.pose.position.y = current_pose.position.y + tool_vector_x.y * PULL_DISTANCE + tool_vector_z.y * KEEP_CUTTING_DISTANCE; 
            virtual_attractor.pose.position.z = current_pose.position.z + tool_vector_x.z * PULL_DISTANCE + tool_vector_z.z * KEEP_CUTTING_DISTANCE; 
        }
        // Pull up in the direction of the tool
        else {
            virtual_attractor.pose.position.x = current_pose.position.x - tool_vector_x.x * PULL_DISTANCE + tool_vector_z.x * KEEP_CUTTING_DISTANCE;
            virtual_attractor.pose.position.y = current_pose.position.y - tool_vector_x.y * PULL_DISTANCE + tool_vector_z.x * KEEP_CUTTING_DISTANCE;
            virtual_attractor.pose.position.z = current_pose.position.z - tool_vector_x.z * PULL_DISTANCE + tool_vector_z.x * KEEP_CUTTING_DISTANCE;
        }
