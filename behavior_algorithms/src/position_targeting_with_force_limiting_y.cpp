...
int main(int argc, char** argv) {
    ...
    */
    ...
    double start_position = abs(current_pose.position.z);
    ...
    */
    while( (loops_so_far <= total_number_of_loops) && !effort_limit_crossed && (((abs(current_pose.position.z) < target_position) && (TARGET_DISTANCE >= 0)) || ((abs(current_pose.position.z) >= target_position) && (TARGET_DISTANCE < 0))) ) {
        // ROS: for communication between programs
        ros::spinOnce();
    ...
    if(effort_limit_crossed) {
        ...
        virtual_attractor.pose = current_pose;
        if (abs(current_pose.position.z) < target_position){
            virtual_attractor.pose.position.z = current_pose.position.z + KEEP_CONTACT_DISTANCE;
        }
        else {
            virtual_attractor.pose.position.z = current_pose.position.z - KEEP_CONTACT_DISTANCE;
        }
    ...
    if((abs(current_pose.position.z) >= target_position && (TARGET_DISTANCE > 0) ) || (abs(current_pose.position.z) < target_position && (TARGET_DISTANCE <= 0) ) ) {
        // Print message
