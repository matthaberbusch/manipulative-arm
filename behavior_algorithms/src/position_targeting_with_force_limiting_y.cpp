...
int main(int argc, char** argv) {
    ...
    while( (loops_so_far <= total_number_of_loops) && (abs(ft_in_robot_frame.force.z) < FORCE_TRESHOLD) && (((abs(current_pose.position.z) < target_position) && (TARGET_DISTANCE >= 0)) || ((abs(current_pose.position.z) >= target_position) && (TARGET_DISTANCE < 0))) ) {
        // ROS: for communication between programs
        ros::spinOnce();
        ...
        cout<<"Current position: "<<endl<<abs(current_pose.position.z)<<endl;
    }
