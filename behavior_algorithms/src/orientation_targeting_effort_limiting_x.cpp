...
int main(int argc, char** argv) {
    ...
    *///                                                                      was  0.1
    ...
    nh.param("/otel_x/target_orientation", TARGET_ORIENTATION, 1.57);
    nh.param<std::string>("/otel_x/param_set", param_set, "Peg");

    // clear parameter from server 
    nh.deleteParam("/otel_x/target_orientation");
    nh.deleteParam("/otel_x/param_set"); 
    ...
    // Maybe also change thresholds for cutting?
    ...
    ROT_MAT_MOVE(1,2) = -sin(ROTATE_ANGLE);
    ROT_MAT_MOVE(2,0) = 0;
    ROT_MAT_MOVE(2,1) = sin(ROTATE_ANGLE);
