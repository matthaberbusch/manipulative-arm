// Modification of PTEL to be used solely for cutting, with higher compliancy to cut on uneven and tilted surfaces 
...
int main(int argc, char** argv) {
    ...
    ros::init(argc,argv,"compliant_cutting");
    ros::NodeHandle nh;
    ...
    srv.request.name = "compliant_cutting";
    ...
    KEEP_CUTTING_DISTANCE: The distance that the tool will be pulled down into the task frame while cutting to ensure that there is sufficient force and contact to cut through the material, larger gives more force, smaller gives less force
    ...
    */
    //                     0.006 before, pulling forward harder now to go over bumps 
    double PULL_DISTANCE = 0.009, KEEP_CONTACT_DISTANCE = 0, DT = 0.01, FORCE_THRESHOLD = 7, TARGET_DISTANCE = 0.01; 
    double NONDIRECTIONAL_FORCE_THRESHOLD = 7, KEEP_CUTTING_DISTANCE = 0.0012; // increase
    ...
    // We will always be in cutting, so the parameter will not be used from the command line (REMOVE THE EXTRA BITS)
    
    // ROS: Get parameter passed in 
    nh.param("/compliant_cutting/target_distance", TARGET_DISTANCE, 0.01);

    // clear parameter from server 
    nh.deleteParam("/compliant_cutting/target_distance"); 
   ...
   // ROS: Wait until we have position data. Our default position is 0.
    ...
    // While we are cutting, we want to move in the direction of the x vector projected on the task plane
